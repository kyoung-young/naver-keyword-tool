...
function generateSignature(timestamp, method, uri, secretKey) {
  // 네이버 공식 signaturehelper.py 기준:
  // message = "{timestamp}.{method}.{uri}"  ← 점(.) 구분자, 개행(\n) 아님
  // key = bytes(secret_key, "utf-8")        ← 문자열 그대로 UTF-8
...
function buildHeaders(method, uri, customerId, accessLicense, secretKey) {
  ...
  // 네이버 공식 Python SDK 기준: signature는 쿼리스트링 제외한 path만 사용
  // requests.get(url, params=params, headers=get_header('GET', '/keywordstool', ...))
